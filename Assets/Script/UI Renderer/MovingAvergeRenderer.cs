...
public class MovingAvergeRenderer : Graphic
{
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
        ...
        for (int i = 0; i < movingAverage.Length; i++)
        {
            ...
            {
                float candleWidth = GraphManager.getCandleWidth;
    ...
    private void drawLine(int i, VertexHelper vh, float angle)
    {
        ...
        vertex.position += new Vector3(GraphManager.getCandleWidth * (i + timeFrame), GraphManager.convertCoordinateYToViewportY(movingAverage[i]));
        vh.AddVert(vertex);
